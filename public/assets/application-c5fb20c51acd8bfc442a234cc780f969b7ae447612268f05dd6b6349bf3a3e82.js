...
//	.value(function(d) { return d.duration; });
	...
	data2 = spark_times;
	hierarchy.nodes(data2);
	x.domain([0, data2.value]).nice();
	down(data2, 0);
    });
    ...
    function down(d, i) {
	h = d.children.length *1.2*y, // height
	d3.select("#barplot")
	    .attr("height", h + m[0] + m[2])
	...
	x.domain([0, d3.max(d.children, function(d) { return d.duration; })]).nice();
	...
	exitTransition.selectAll("rect").attr("width", function(d) { return x(d.duration); });
	...
	// Update the x-scale domain.
	x.domain([0, d3.max(d.parent.children, function(d) { return d.duration; })]).nice();
...
function initIndTree() {
	...
	var height = Math.max(10, nodes.length * barHeight + margin.top + margin.bottom);
